...
import 'package:granio_dart_api/application/helpers/crypt_helper.dart';
import 'package:injectable/injectable.dart';
import 'package:mysql1/mysql1.dart';

import 'package:granio_dart_api/application/database/i_database_connection.dart';
import 'package:granio_dart_api/application/exceptions/credentials_not_found_exception.dart';
import 'package:granio_dart_api/application/exceptions/database_exception.dart';
import 'package:granio_dart_api/application/exceptions/invalid_recover_4digit_code_exception.dart';
import 'package:granio_dart_api/application/exceptions/user_exists_exception.dart';
import 'package:granio_dart_api/application/exceptions/user_not_found_exception.dart';
import 'package:granio_dart_api/application/helpers/random_hash_coder_helper.dart';
import 'package:granio_dart_api/entities/enums/custom_platform.dart';
import 'package:granio_dart_api/entities/user.dart';
...
class UserRepository implements IUserRepository {
  ...
  Future<void> changeUserPassword(String email, String newPassword) async {
    ...
    try {
      ...
      await conn.query(
        'UPDATE `granio_db`.users SET `password` = (?) WHERE `email` = (?)',
  ...
  Future<User> createUser(User user) async {
    ...
    try {
      ...
      User newUser = User(
        ...
        telephone: createdUser['telephone'] as String,
        ...
        //         : null,
        profileImage: createdUser['profile_image'] != null
            ? Uint8List.fromList(
  ...
  Future<String> deleteUser(int id, User contaConectada) async {
    ...
    try {
      ...
      await conn.transaction((_innerConn) async {
        await Future.wait([
          _innerConn.query(
            'DELETE FROM `granio_dart`.user WHERE `email` = (?)',
  ...
  Future<User> findById(int id) async {
    ...
    try {
      ...
      var myResults = await conn.query(
        '''
          SELECT * from `granio_db`.users WHERE `id` = (?)
          ''',
        [id],
      );
      ...
      if (myResults.isEmpty) {
        ...
        return User(
          ...
          profileImage: dataMySql['profile_image'] != null
              ? Uint8List.fromList(
                  (dataMySql['profile_image'] as Blob).toBytes(),
                )
              : null,
          dataCriacaoConta: dataMySql['creation_date'],
        );
      }
    } on MySqlException catch (e, s) {
    ...
    try {
      ...
      if (myResults.isEmpty) {
        ...
        return User(
          ...
          name: userSqlData['name'] as String,
          email: userSqlData['email'] as String,
          // cpf: userSqlData['cpf'] as String,
          telephone: userSqlData['telephone'] as String,
          password: userSqlData['password'] as String,
          ...
          //         )
          //         : null,
          profileImage: userSqlData['profile_image'] != null
              ? Uint8List.fromList(
  ...
  Future<void> sendRecoveryPasswordEmail(
    ...
    try {
      ...
      var myResults = await conn.query(
        'SELECT * FROM `granio_db`.users WHERE `email` = (?)',
        [userEmail],
      );
      ...
      final message = Message()
        ...
        ..html = '''
        <h2>Olá</h2>
        <p>Utilize o código abaixo para recuperar a sua senha no aplicativo granio referente a conta do seu email</p>
        ...
        <p>Obrigado,</p>
        <p>Equipe de Desenvolvimento da granio.</p>
        ''';
  ...
  Future<void> updateDeviceToken(
    ...
    try {
      ...
      final query = '''
        UPDATE `granio_db`.users SET $set WHERE `id` = (?)
      ''';
  ...
  Future<void> updateRecoverTokenOnDatabase(
    ...
    try {
      ...
      await conn.query(
        'UPDATE `granio_db`.users SET `pass_recovery_token` = (?) WHERE `email` = (?)',
  ...
  Future<void> updateRefreshToken(User user) async {
    ...
    try {
      conn = await connection.openConnection();
      await conn.query(
        'UPDATE `granio_db`.users SET refresh_token = (?) WHERE `id` = (?)',
  ...
  Future<void> validateRecoveryPasswordFourDigitCode(
    ...
    try {
      ...
      var compareCodes = await conn.query(
        'SELECT * FROM `granio_db`.users WHERE `email` = (?) AND `pass_recovery_token` = (?)',
        [email, code],
      );
  ...
  Future<void> updateValidateEmailCodeOnDatabase(
    ...
    try {
      ...
      await conn.query(
        'UPDATE `granio_db`.users SET `token_email_confirm` = (?) WHERE `email` = (?)',
        [random4DigitCode, userEmail],
      );
    } on MySqlConnection catch (e, s) {
  ...
  Future<void> validateConfirmEmailFourDigitCode(
    ...
    try {
      ...
      var compareCodes = await conn.query(
        'SELECT * FROM `granio_db`.users WHERE `email` = (?) AND `token_email_confirm` = (?)',
        [email, code],
      );
      ...
      await conn.query(
        '''
          UPDATE `granio_db`.users SET `token_email_confirm` = (?), `email_confirmed` = (?) WHERE `email` = (?)
        ''',
  ...
  Future<void> sendValidateEmailCode(
    ...
    try {
      ...
      final message = Message()
        ..from = Address(adminGmail, 'granio')
        ..recipients.add('$userEmail')
        ..subject = 'granio - Validar Email'
        ..html = '''
        <h2>Olá</h2>
        <p> Digite no aplicativo granio o seguinte código para validar sua conta e terminar seu cadastro: </p>
        <p style="font-size: 18px;">Código: <strong style="color: rgb(23, 30, 56); font-size: 24px;">$random4DigitCode</strong></p>
        <p>Se você não solicitou a redefinição da sua senha, ignore este e-mail.</p>
        <p>Obrigado,</p>
        <p>Equipe de Desenvolvimento da granio.</p>
        <img src="https://granio-prod.s3.us-east-2.amazonaws.com/uploads/granio_fundo_azul.jpg">
        ''';
